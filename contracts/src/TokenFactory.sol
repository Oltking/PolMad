...
contract TokenFactory {
    ...
    function createToken(
        ...
        if (address(trustRegistry) != address(0)) {
            // NOTE: this string is deliberately "polymad", not "polmad". It was
            // fixed at deploy time and is an input to a hash, so changing it would
            // alter the bytecode and break source verification for the live
            // factory — while being invisible to users. Kept as-is on purpose.
